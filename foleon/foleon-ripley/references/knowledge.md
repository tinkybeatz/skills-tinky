...
- 2026-09-11 · Editor · the grid dev controls bar never appears even with `X-EDITOR-GRID` set in localStorage — `useGridLayoutControlsEnabled` ANDs two gates: the Unleash flag `editor.gridLayoutSettings` on the account AND `localStorage['X-EDITOR-GRID'] === 'true'`; PR bodies tend to mention only the localStorage half. The flag is per account in Overlord's "Features (Unleash toggles)" panel; localStorage is per origin, so it does NOT carry from staging to `https://localhost:3000` · refs: packages/foleon-core-editor/src/@entities/grid-layout/grid-layout.dev-controls.ts, @shared/flags.ts:48 · sheet: no
- 2026-09-11 · Editor · the swap-element button is absent from an element's options bar while Settings/Replace/Duplicate render fine — `OptionsButtons.SwapElement` renders only when `hasAllowSwapElements && isPrimaryScreensize`, i.e. the account-level `allowSwapElements` privilege (Overlord entitlements table, not the Unleash toggles) plus the xl/Desktop viewport. The per-entity `CustomDisabled role` wrappers are a separate, weaker gate, which is why sibling buttons still show · refs: packages/foleon-core-editor/src/@shared/options-buttons-new/index.tsx:493-505, @auth/privilege/privilege.model.ts:134 · sheet: no
- 2026-09-11 · Build & tooling · `pnpm install -r` fails with `ERR_PNPM_FETCH_403` on `europe-west4-npm.pkg.dev/foleon-staging/...` — the Google Artifact Registry token baked into `.npmrc` has expired, not a permissions problem. Re-run `sh scripts/prepare-auth.sh` (copies `.npmrc.template`, re-runs `npx google-artifactregistry-auth`) then install again. Also note `packages/*/dist` can be months stale after a branch switch: `@foleon/core` ships grid code, so run `pnpm build` before `pnpm editor:dev` or the editor runs against old core · refs: scripts/prepare-auth.sh, package.json:8 · sheet: no
- 2026-09-14 · Editor · A drag-and-drop guard keyed on `type === 'sidebar-item'` silently skips hotspot/carousel/gallery/slider — sidebar drags do NOT all carry `type: 'sidebar-item'`. `getActiveDragTypeByIdentity` (a plain local function at the BOTTOM of `views/sidebar/components/side-bar-element.tsx`, not exported, not in `drag-and-drop-utils.ts`) maps identity to a per-identity DragType: `hotspot`→`sidebar-item_hotspot`, `carousel`/`gallery`/`slider`→`sidebar-item_<identity>`, `block`/`reveal`→`sidebar-block-item`, `column`→`sidebar-column-item`, `fragment-link`→`sidebar-fragment-item`, everything else→`sidebar-item`. `isSidebarItemDragData()` deliberately accepts the whole `sidebar-item*` family, so `isSidebarItemDragData(item)` and `item.type === 'sidebar-item'` are NOT interchangeable — the first catches hotspots/widgets, the second does not. Use the exact-type comparison when a rule must spare hotspots and the carousel/gallery/slider widgets (they create a SIBLING block rather than dropping inside the target). Same file: the clipboard panel reuses this exact drag path (`side-bar-element.tsx` `end()` branches on `if (id)` → `pasteElementFromClipboard`/`pasteBlockFromClipboard`/`pasteColumnFromClipboard`/`pasteHotspotFromClipboard`), so a drop rule written for the entity panel automatically covers "paste from clipboard" — there is no separate paste-onto-canvas path to guard. · refs: `views/sidebar/components/side-bar-element.tsx` · `@common/drag-and-drop-new/drag-and-drop-utils.ts` · `@entities/clipboard/clipboard.mutations.tsx` · PROD-4791 · sheet: none
- 2026-09-14 · Editor · Block refuses a drop but the user sees no feedback — react-dnd only calls `hover`/`canDrop` on a target whose `accept` list contains the drag type, so a block can only REFUSE VISIBLY what it first ACCEPTS. Pattern used in `block.editor.tsx`: add the type to `accept` (e.g. `...(getHasGridChild(entity) ? ['column-child'] as const : [])`) so `handleBlockHover` can return `{ code: DropLocation.notAllowed, message }`, while `handleBlockCanDrop` returns false so nothing lands. The two must be kept in sync by hand — nothing enforces it. Also: the `accept` array in `block.editor.tsx` is a 4-way nested ternary keyed on `isLocked` → `refs.card || refs.slider` → `numberOfColumns === 0` → else; `getNumberOfColumns` (`block.model.ts`) counts columns under the block's `row` ref, so a grid block (refs hold `grid-layout`, no `row`) scores 0 and lands in the third branch. · refs: `@entities/block/block.editor.tsx` · `@entities/block/block.model.ts` · `@entities/block/drag-and-drop/drag-and-drop.hook.tsx` · PROD-4791 · sheet: none
- 2026-09-14 · Tests & Playwright · `npx vitest run <file>` fails with "Failed to load url @common/..." — the editor's vitest config is NOT at the package root; aliases live in `src/vite.config.ts`. Run `npx vitest --config src/vite.config.ts run <files>` from `packages/foleon-core-editor` (that is what `pnpm --filter @foleon/editor test` does). A bare `npx vitest run` resolves no `@common`/`@entities`/`@lib` alias and reports every suite as "0 test" / Failed Suite, which reads like a broken test rather than a missing config. Also: `timeout` is not present on this macOS shell — prefixing a test command with it aborts with "command not found" and exit code 0, i.e. a FALSE GREEN. · refs: `packages/foleon-core-editor/src/vite.config.ts` · `packages/foleon-core-editor/package.json` · sheet: none
