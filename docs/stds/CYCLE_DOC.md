...
`status` prints the state. Nothing measures how long a topic has been shaping: the
maintainer shapes one topic at a time, so elapsed days say nothing about the topics they are not on
`[SRC-030]`.
...
- A hook **MAY** additionally **report** a **time-based** signal that the doc's own dates make
  computable — which week of the dev window today is, or that window nearing its end — on the same
  report-only terms as the queue. Such a nudge **MUST NOT** write anything, and **MUST NOT** imply or
  propose a hill position: CYC-6 names *elapsed time* among the things a hill may never be derived
  from. `[SRC-018]` A signal about how long one **topic** has been shaping is **MUST NOT** territory,
  not `MAY`: it measures the wrong thing (CYC-3). `[SRC-030]`
...
time-based nudge is admitted on the same terms: the dev window is only useful while there is cycle
left to react in, and a signal the tool computes but never volunteers is one the maintainer learns
too late to act on.
...
- `[SRC-030]` Internal — the maintainer's report, 2026-08-31, on reading the Notion mirror of Cycle-11: *"'still shaping after 14 of ~14 days of its appetite — escalate to whoever can answer the open questions rather than researching further' -> this is stupid. we don't care about the metric. i started by only working on 1 shape. get rid of the 'still shaping' blablah. I am human, i work on 1 shape at a time."* The measurement was wrong at the root, not merely noisy: `shaping_overrun` counted days from the **cycle start** because the doc records no date a topic entered shaping, so a topic that had not been opened yet accrued overrun at the same rate as the one being worked. On the live cycle both shaping topics were flagged, and neither had been touched since intake.
...
| Version | Date | Change |
|---|---|---|
| **7.0.0** | 2026-08-31 | **The shaping circuit breaker is removed, because it measured the wrong thing.** v1.6.0 borrowed Shape Up's circuit breaker and applied it to shaping: past roughly a quarter of a topic's appetite still unshaped, escalate rather than research further. The rule was sound and the measurement was not. The doc records no date a topic *entered* shaping, so `shaping_overrun` counted from the **cycle start** — which is only equivalent when every topic is being shaped in parallel, and this maintainer shapes one at a time `[SRC-030]`. The observable consequence: on the live cycle both shaping topics were flagged as overrun while neither had been opened since intake, on the Notion page and at every session start. A nudge that is wrong about the topic nobody is working on is not a weak signal, it is a false one, and it sat next to the true nudges. CYC-3 loses the `SHOULD`; its enforcement line now says explicitly that nothing measures shaping duration, so the clause is not reintroduced by someone reading the gap as an oversight. CYC-11's time-signal allowance narrows from "a topic past its shaping budget, or the dev window" to the dev window alone, and names the shaping variant as forbidden rather than merely unmentioned. What the rule was *for* survives in the hold/split/shape decision (`references/shaping.md`), which is the judgement the metric was standing in for. MAJOR: an implementation that emitted the warning conformed at 6.3.0 and does not at 7.0.0; migration is deleting `shaping_overrun`, its two call sites, the hook's report path and its marker files. |
